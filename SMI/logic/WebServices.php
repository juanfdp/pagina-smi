...
class WebServices
{
	...
	public function ObtenerPrecio($IdProducto,$IdAseguradora,$FechaInicioViaje,$FechaFinViaje,$cantidadDias,$ArregloPasajeros,$PrecioSistema)
	{
			...
			switch ($IdAseguradora) {
				...
				case "4E04D2C4-5C91-46E4-99DD-E992638DA6F8": // ASSIST CARD
					...
					if(count($this->polizasAssistCard)==0){//VALIDAMOS QUE EL SERVICIO WEB Y HAYA SIDO CONSUMIDO, PARA NO CONSUMIRLO MAS DE UNA VEZ POR COTIZACION, OPTIMIZACION EN LAS BUSQUEDAS.
						...
						//var_dump( $this->polizasAssistCard);//IMPRESION PARA CONOCER LOS RESULTADOS.
					}
					...
					if(count($this->polizasAssistCard)!=0){
						...
						for($i=0;$i<count($this->polizasAssistCard); $i++){
							...
							//echo $poliza[0]."<br>";//IMPRESION DE LA REFERENCIA DE LA POLIZA
							//echo sprintf($this->polizasAssistCard[$i])."<br>";//IMPRIMOS POSICION A POSICION LOS RESULTADOS EN EL ARREGLO
							$RegistroWebService=explode(trim($poliza[0]), sprintf($this->polizasAssistCard[$i]));
							//echo var_dump($RegistroWebService)."<br>";//IMPRESION PARA CONOCER LOS RESULTADOS DIVIDIR LOS RESULTADOS POR  LA REFERENCIA DE LA POLIZA.
							...
							if(count($RegistroWebService)>=2){//VALIDAMOS QUE LA REFERENCIA DE NUESTRA BUSQUEDA SE ENCUENTRE
								...
								//echo var_dump($Precio)."<br>";//IMPRESION PARA CONOCER LOS RESULTADOS DIVIDIR LOS RESULTADOS POR  LA REFERENCIA DE LA POLIZA.
									
								$PrecioFormatoFinal=sprintf($Precio[124]);
								$PrecioFormatoFinal = $this->func->str2num(strip_tags($PrecioFormatoFinal));									
				...
				case "715697A2-F643-4D5B-AC49-83B11E8546B0": // TRAVEL ACE
					...
					return 	$this->func->str2num($PrecioSistema);
		...
		catch (Exception $e)
		{
			...
			return -1;//RETORNAMOS -1 PARA QUE LA POLIZA NO SEA MOSTRADA EN EL PORTAL.
		}
	}
